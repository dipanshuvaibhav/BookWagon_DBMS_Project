...
session_start();
 ...
 if(isset($_POST['pid'])){
   ...
   if(!$code){
     ...
     $query->bind_param("isssis",$user_id,$p_name,$p_price,$p_image,$p_qty,$p_price);
     $query->execute();
 ...
 if(isset($_POST['action']) && isset($_POST['action']) == 'order'){
   ...
   $data = '';
   $stmt = $conn->prepare("INSERT INTO heroku_adaaf59afa8e08a.order(customer_id,o_name,o_email,o_phone,o_address,o_pmode,o_products,o_amt_paid,o_date) VALUES (?,?,?,?,?,?,?,?,?)");
   $stmt->bind_param("issssssss",$cust_id,$name,$email,$phone,$address,$pmode,$products,$grand_total,$date);
   $stmt->execute();
